...
#else
#include <ncurses.h> // For getch() on Unix-based systems
#endif
...
char getInput() {
    ...
    raw();
    keypad(stdscr, TRUE);
    input = getch(); // Get input from the keyboard on Unix-based systems (e.g., Linux, macOS)
    endwin();
#endif
    return input;
}
...
#else
    system("clear"); // Clear the screen on Unix-based systems (e.g., Linux, macOS)
#endif
}
